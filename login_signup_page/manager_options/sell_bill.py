...
def display_products():
    ...
    for row in sell_db.fetch_products(bill_no):
        products_table.insert('', END, values=(row[0], row[3], row[4], row[5], row[6]), image=delete_btn1)
...
def update_product():
    sell_db.update_product(
        int(update_total_price_entry.get()) - (int(update_count_entry.get()) * int(update_buy_price_entry.get())),
        update_total_price_entry.get(),
        0,
        update_sell_price_entry.get(),
...
def add_product_page():
    global code_entry, name_entry, count_entry, sell_price_entry, buy_price_entry, total_price_entry, total_profit_entry
    ...
    sell_price_lb = Label(win, text='سعر البيع', bg='#ABB2B9', fg='#117A65', font=('tajawal', 12, 'bold'))
    sell_price_lb.place(x=win_width*.8, y=win_height*.44, width=win_width*.18, height=win_height*.07)
    sell_price_entry = Entry(win, font=('tajawal', 9, 'bold'), bd=0, highlightbackground='#117A65', highlightcolor='#117A65', highlightthickness=2, textvariable=product_sell_price, justify=CENTER)
    sell_price_entry.place(x=win_width*.3, y=win_height*.44, width=win_width*.5, height=win_height*.07)
...
def autocomplete():
    global product_code, product_name, product_sell_price, product_buy_price
    ...
    if code_entry.get() != "" and name_entry.get() != "" and pro_db.id_product_to_bill(code_entry.get()) != pro_db.name_product_to_bill(name_entry.get()):
        ...
        if sell_price_entry.get() != "":
            ...
            if data == []:
                ...
                total_price = str(int(sell_price_entry.get()) * int(count_entry.get()))
                total_price_entry.delete(0, END)
                total_price_entry.insert(0, total_price)
                total_profit = int(sell_price_entry.get()) * int(count_entry.get()) - int(rows[4]) * int(count_entry.get())
            ...
            if data == []:
                ...
                sell_price_entry.insert(END, rows[2])
                buy_price_entry.delete(0, END)
                buy_price_entry.insert(0, rows[4])
                total_price = str(int(sell_price_entry.get()) * int(count_entry.get()))
                total_price_entry.delete(0, END)
...
def update_totals():
    total_price = int(update_sell_price_entry.get()) * int(update_count_entry.get())
    ...
    total_profit = total_price - int(update_buy_price_entry.get()) * int(update_count_entry.get())
...
def update_product_page():
    global update_frame, update_code_entry, update_name_entry, update_count_entry, update_buy_price_entry, update_sell_price_entry, update_total_price_entry, update_total_profit_entry
    ...
    update_sell_price_lb = Label(update_frame, text='سعر البيع', bg='#ABB2B9', fg='#117A65', font=('tajawal', 12, 'bold'))
    update_sell_price_lb.place(x=update_frame_width*.8, y=update_frame_height*.44, width=update_frame_width*.18, height=update_frame_height*.07)
    update_sell_price_entry = Entry(update_frame, font=('tajawal', 9, 'bold'), bd=0, highlightbackground='#117A65', highlightcolor='#117A65', highlightthickness=2, textvariable=product_sell_price, justify=CENTER)
    update_sell_price_entry.place(x=update_frame_width*.3, y=update_frame_height*.44, width=update_frame_width*.5, height=update_frame_height*.07)
    update_sell_price_entry.delete(0, END)
    update_sell_price_entry.insert(0, item_data[1])
    ...
    update_buy_price_entry.insert(0, buy_price)
...
def update_total_change_page():
    ...
    for item in sell_db.fetch_products_info(bill_no):
        ...
        total_profit += int(values[0][0]) * int(values[0][2]) - int(values[0][1]) * int(values[0][2])
...
products_table = ttk.Treeview(view_frame,
                        columns=('total_price', 'sell_price', 'count', 'products', 'product_no'))
...
products_table.heading('sell_price', text='سعر البيع')
...
products_table.column('sell_price', width=int(view_frame_width*.15), anchor=CENTER)
